...
import useEpisodeSheet from "@/view-models/tv/episode-sheet"
...
export default function EpisodeSheet({
  ...
  const viewModel = useEpisodeSheet({
    onCreate: onCreate,
    tvShowID: tvShowID
  })
  ...
  return (
          ...
          ? {
            ...
            disabled: (
              viewModel.title.value.length <= 0 ||
                ...
                viewModel.episodeNumber.value.length <= 0 ||
                viewModel.seasonNumber.value.length <= 0 ||
    ...
    >
      ...
      />
      <Text
        textKey={mode === "creation" ? "New Episode" : "Details"}
...

      {
        viewModel.isError && (
          <Text
            textKey="Unable to create episode. Try again later."
